...
* Integrador: Borra la seva còpia local de la branca de treball del programador (els canvis ja estan integrats a master). Es fa servier `git branch -d -r nom_de_la_branca_remota` per borrar una remote tracking branch.
