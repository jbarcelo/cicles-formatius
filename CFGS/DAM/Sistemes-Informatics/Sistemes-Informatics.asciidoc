...
Descriu les següents utilitats de xarxa
...
* Fes un backup entre dos directoris del teu sistema d'arxius. `rsync -av --delete directori1 directori2`. Esbrina què signinica cada un dels flags.
* Fes un login remot amb `ssh usuari@equip`. Pots instal·lar un servidor ssh en una màquina virtual linux per connectar-t'hi.
* Fes un backup entre dos equips en xarxa. `rsync -av --delete directori1 usuari@equip:directori2`. 
* Crear un parell de claus pública/privada per a connexions ssh. `ssh-keygen -t rsa`
* Instal·la la teva clau pública en el servidor. `ssh-copy-id usuari@equip`
* Fes una connexió ssh de prova.
* Fes un backup en xarxa fent servir un parell de claus privada i pública per ssh.
* Programa un crontab per a que faci un backup a una determinada hora. Comprova que la còpia realment s'actualitza a l'hora fixada.
